...
import type { AstIfStmt } from "../Ast/AstIfStmt";
...
export function parseIfStmt(code: string): ParseData{
    let result:ParseData
    let remain = code;
    const kw = parseKeyword(remain);
    if(kw === undefined || kw.deliverable.nodeType !== "KEYWORD" || kw.deliverable.name !== "if"){
        return undefined;
    }
    remain = kw.remain;
